...
\begin{algorithm}[H]
\caption{Circuit FDKG($|\GuardianSetOf{}|$ = guardian\_set\_size, t = threshold)}
...
\Assert \PartialEncryptionKey{i} = \EscalarMulFix{\PartySecretKey{i}, \Base}\;
...
\For{i \KwIn \GuardianSetSize}{
    ...
    F $\leftarrow$ \EscalarMulFix(r2_i, \Base)\;
    
    J $\leftarrow$ \BabyAdd(R, F)\;
    \Delta $\leftarrow F_x - \Eval[i][\Threshold]$\;

    \Assert $\EncryptedPartialDecryptionKeyShare{}{i}.C1 = \EscalarMulFix{r1[i], \Base}$\;
...
\begin{algorithm}[H]
\caption{Circuit EncryptedBallot(voters, options, m)}
