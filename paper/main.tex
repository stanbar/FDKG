...
\newcommand{\ProofFDKGInformal}{"Given \EncryptedPartialDecryptionKeyShare{i}{j} and $\Party{j} \in \GuardianSetOf{i}$, I know $f_i = a_0, \dots, a_{t-1}$, $r1_1,\dots,r1_k$, and $r2_1,\dots,r2_k$, s.t. the \EncryptedPartialDecryptionKeyShare{i}{j} is an encrypted value of a polynomial $f_i(\cdot)$ applied to $j$"}
...
\newcommand{\PartialDecryptionFrom}[1]{\ensuremath{\TotalA \cdot d_{#1}}}
...
\begin{enumerate}
    ...
    \item For each received encrypted share $\EncryptedPartialDecryptionKeyShare{j}{i} \in \{ \EncryptedPartialDecryptionKeyShare{j}{i} | \Party{j} \in \mathbb{D} \setminus \{\Party{i}\} \text{ and } \Party{i} \in \GuardianSetOf{j}\}$ \begin{enumerate}
        ...
        \item Compute a zero-knowledge proof \ProofPD{i}{j} (described in Section~\ref{sec:proof-pd}).
...
\begin{itemize}
    ...
    \item $\vec{s}=[\PartialEncryptionKey{i}, (x,y)_1,\dots,(x,y)_k, \EncryptedPartialDecryptionKeyShare{i}{1},\dots, \EncryptedPartialDecryptionKeyShare{i}{k}]$
...
\Assert{guardian\_set\_size $\geq$ threshold}\;


\BlankLine

\PartySecretKey{i} $\leftarrow$ coefficients[0]\;

\Assert \PartialEncryptionKey{i} = \EscalarMulFix{\PartySecretKey{i}, BASE8}\;

votingPublicKey[0] $\leftarrow$ C1[0]\;
votingPublicKey[1] $\leftarrow$ C1[1]\;

\For{$i \gets\ 0$ \KwTo guardian\_set\_size}{
    $eval[i][0] \leftarrow coefficients[0]$\;
    \For{$j\leftarrow 1$ \KwTo threshold-1}{
        e $\leftarrow (i+1)^j\ \%\ \BABYORDER$\;
        mul $\leftarrow (coefficients[j]  e)\ \%\ \BABYORDER$\;
        eval[i][j] $\leftarrow (eval[i][j-1] + mul)\ \%\ \BABYORDER$\;
    }
    
    rP $\leftarrow$ \EscalarMulAny{\Party{i}, r1[i]}\; % party in guardiansPubKeys[i]
    
    randomPoint $\leftarrow$ \EscalarMulFix(r2, BASE8)\;
    
    (x,y) $\leftarrow$ \BabyAdd{rP, randomPoint[1]}\;
    \Delta $\leftarrow$ randomPoint[0] - eval[i][threshold-1]\;

    \Assert \EncryptedPartialDecryptionKeyShare{j}{i}[1] = \EscalarMulFix{r1[i], BASE8}\;
    \Assert \EncryptedPartialDecryptionKeyShare{j}{i}[2] = (x,y)\;
    \Assert \EncryptedPartialDecryptionKeyShare{j}{i}[\Delta] = \Delta\;
}
\end{algorithm}

\begin{algorithm}[H]
\caption{Circuit PVSS(guardian set size, threshold)}
\label{alg:circuit_pvss}

\KwIn{Statement $\vec{s}$: (\PartialEncryptionKey{i}, (x,y)_1,\dots,(x,y)_k, \EncryptedPartialDecryptionKeyShare{i}{1},\dots, \EncryptedPartialDecryptionKeyShare{i}{k})}
\KwIn{Witness $\vec{w}$: (a_0, \dots, a_{t-1}, r1_1,\dots,r1_k, r2_1,\dots,r2_k)}
...
\begin{algorithm}[H]
\caption{Circuit EncryptedBallot(voters, options)}
...
\begin{definition}[Lock-in] \label{def:lock-in}
    A Lock-in state occurs in an FDKG system if there exists a participant $\Party{i} \in \mathbb{D}$ for whom neither the full partial decryption $\PartialDecryptionFrom{i}$ is published by \Party{i} itself nor at least $t$ members of its guardian set $\mathbb{G}_i$ publish their shares of partial decryption $\SharePartialDecryptionFromTo{i}{j}$. Formally:
    \[
