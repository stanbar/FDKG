...
\begin{figure}
    ...
    \caption{Time required to solve DLP with respect to number of voters and number of candidates.}
...
Analysis of the measurements shows that time required to find the DLog follows the power-law $Y=aX^b$ where parameter $b$ is linear with the number of options, which is an expected behaviour. In other words, the time to compute the results is linear with the number of voters and exponential with the number of candidates.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{stack-bc.png}
    \caption{Possible protocol deployments.}
    \label{fig:stack-bc}
\end{figure}
...
Our experimental evaluation was conducted on a MacBook Pro with an Apple M1 Pro processor and 16GB of RAM. The experiments focused on measuring the execution time and message size for both the Plonk and Groth16 proving systems. The results provide valuable insights into the practicality and efficiency of the proof systems in the context of our protocol.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{dlog-search-time2.png}
...
In conclusion, while our protocol presents a significant step forward in secure and decentralized voting, further research is required to address its computational demands and scalability challenges. Future work could focus on optimizing zkSNARKs implementations, possibly batching proofs together, exploring alternative cryptographic schemes, and enhancing the protocol's adaptability to diverse election scales and requirements.
