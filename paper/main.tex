...
% The offline tally phase of the protocol involves solving the Discrete Logarithm Problem (DLP) to extract the number of voters for each candidate. This process, depicted in Figure~\ref{fig:dlog-search}, reveals that the time required to solve DLP aligns with a power-law relationship \( Y = aX^b \), where \( b \) varies linearly with the number of options. The computation time is linear with the number of voters, but increases exponentially with the number of candidates. This relationship presents scalability challenges for larger elections, where the number of candidates can significantly impact the time required for tallying.
