...
% \author{Stanislaw Baranski\inst{1}\orcidID{0000-0001-7181-8860} \and
% Julian Szymanski\inst{1}\orcidID{1111-2222-3333-4444}}
% %
% \authorrunning{S. Baranski and J. Szymanski}

% \institute{Gdansk University of Technology, Narutowicza 11/12, 80-233, Gdansk, Poland
% \email{\{stanislaw.baranski,julian.szymanski\}@pg.edu.pl}}
...
\begin{abstract}
Voting is a fundamental tool for digital democracy. The current landscape of internet voting protocols consists of trusted centralised servers, private networks or costly public blockchains. This leaves a void for small organisations that want a secure and free tool for democratic decision-making. 

This paper introduces PeerVote, an Internet voting protocol that fills this gap by enabling secure and free voting without trusted third parties. PeerVote uses Federated Distributed Key Generation (FDKG), which ensures resilience to node unavailability and provides a flexible trust model, making it suitable for practical use.
...
% The use of zero-knowledge proofs ensures the voting process is private, verifiable and secure, allowing voters to cast confidential votes while detecting incorrect messages. 
...
Voting serves as a fundamental mechanism for collective decision-making and is used in contexts ranging from non-governmental organisations and corporate boardrooms to national presidential elections and global online polls.
...
\newcommand{\ProofBALLOT}[1]{\ensuremath{\textrm{PROOF}_{#1,\textrm{B}}}}
...
In the online tally phase, the subset of parties $\Tallies \subseteq  \Parties$ engaged in Threshold ElGamal Decryption includes at least $t$ parties from every guardian set \GuardianSetOf{1},\dots,\GuardianSetOf{|\SetOfFDKG|}. For each party $\Party{i} \in \mathbb{T}$:
...
We have made our protocol's implementation in TypeScript and Golang open-source, facilitating widespread deployment and enhancing accessibility. The codebase is available at \url{https://github.com/anonymised} % \url{https://github.com/stanbar/v2v-private-voting}.
